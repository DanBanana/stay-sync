...
public class GetBookingsByRoomQueryHandlerTests
{
    ...
    public async Task Handle_ReturnsBookings_WhenOwner()
    {
        ...
        var handler = new GetBookingsByRoomQueryHandler(ctx, currentUser);

        var result = await handler.Handle(new GetBookingsByRoomQuery(room.Id), CancellationToken.None);

        result.Should().HaveCount(1);
        result[0].ExternalUid.Should().Be("uid-1");
    }
    ...
    public async Task Handle_ThrowsForbiddenException_WhenNotOwner()
    {
        ...
        await Assert.ThrowsAsync<ForbiddenException>(() =>
            handler.Handle(new GetBookingsByRoomQuery(room.Id), CancellationToken.None));
    }
}
